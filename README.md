...
In lvmdatasimulator we use the specific format of the Cloudy models that is described below. If someone wants to run simulations using their own model grids, 
they can save such grids in the same format and rewrite the path to the model grid in the config file
...
ISM component generated by the lvmdatasimulator (source field) is saving to the multi-extensions fits file. 
It is possible to externally modify the file before running simulator. Every nebula in this file has their own ID. Following extension are possible in the file:
- Primary extension contain main parameters of the ISM object (size, wcs, etc.)
...
- *Comp_{ID}\_Fluxratios* - Fluxes in emission lines to be convolved with the brightness and velocity distribution. This is used if the spectrum doesn't change accross the nebula. 
It has size of n_lines x 2 elements, where the first row show the exact wavelength of the line, and the second row - its flux (normalized to Halpha or any other line corresponding to *Comp_{ID}\_Brightness*). 
...
Primary extension contains following important fields:
...
Headers of each other extensions can contain the following fields:
